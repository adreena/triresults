class Race
  ...
  def self.upcoming_available_to (racer)
  	 all_upcoming_races = Race.upcoming
  	 racer_upcoming_races = racer.races.upcoming.pluck(:race).map {|r| r[:_id]}
  	 uregistered_races= all_upcoming_races.where(:id.nin => racer_upcoming_races).map{|r| r.id}

  	
  	 return Race.in(:id=>uregistered_races)
  end
end
